...
private:
// все start.. 
	stack<std::string> stackDigitBool;//конечный стек,эл-ты строки,когда уже есть ОПЗ,в нем могут быть цифры,бул переменные,и тд
	stack<char> operation;//стек ,нужный для дейкстры
	bool startDigit = false;
	std::string tempForDigits;//временное для чисел
	std::string tempForLetters;//временное для букв
	std::string opz;//постоянный для опз,выводим его в конце
	double digitForStack = 0.0;//перевод из строки в число
	long int lenghtOfDigit = 0;//длина строки с числами,std::string tempForDigits;
	int countOpenBracket = 0;//кол-во открытых скобок,если доб  открывающаяся скобка+,и наоборот
	bool startLetters = false;//для тру или фолс
	bool startSymbols = false;//если он тру,а придет **,то он ошибку выдаст
	bool startCloseBracket = false;//закрывающиеся скобки
	bool operandDigit = false;//получилось число,а пришел +,мы должны зафиксировать число
	bool operandBool = false;//фиксируем тру или фолс
	bool startPoint = false;//если числа не целые,а вещественные
	bool startTilda = false;//отрицание
	std::queue<std::string> queueOpz;//очередь,с разными типами данных в строковом формате соответ. опз
...
public:
	...
	void printCommands(char ch)//пишет команды,
	{
		if (ch == '^')
		{
	...
	void dkstr(char c)//опрелеляет ,как засовывать и выпихивать из стека операции
	{
	...
	int separator(char ch)//считывает из файла
	{
		if (startDigit == true)
		{
			...
			else
			{
				...
				digitForStack = atof(tempForDigits.c_str());
	...
	std::string result()
	{
		...
		while (queueOpz.empty() == false)
		{
			...
			{
				operand_2 = atof(stackDigitBool.top().c_str())*(-1);
				stackDigitBool.pop();
